...
What makes WildFly stand out in this context is that it goes beyond just running the application code. Its embedded web server, \textbf{Undertow}, handles everything that a reverse proxy like Nginx would normally be responsible for: serving the frontend's static files, the 404-to-\texttt{index.html} fallback that client-side routing needs, TLS termination, HSTS, CORS, and security headers. The two application modules --- the IAM and the API --- are deployed as WAR files inside the same WildFly instance, and database connections are managed through its datasource subsystem. The result is a single platform that replaces what would otherwise be three or four separate components to configure and maintain.
...
The application is delivered as a Progressive Web App, which is covered in the next subsection.

\subsection{Progressive Web App}

A Progressive Web App (PWA) is a web application that uses standard browser APIs to behave
closer to a native application --- without going through an app store or requiring any
installation beyond a browser prompt. The two core building blocks are the
\textbf{Service Worker} (a background script the browser runs separately from the page) and
the \textbf{Web App Manifest} (a JSON file that describes the app's name, icon, and
display mode).

For a team-facing tool like Canny TMS, two PWA capabilities matter in practice.

\textbf{Offline access.} The caching strategy chosen here is \textit{Network with Cache
Fallback}: the Service Worker intercepts every request and tries the network first. If the
network is not reachable, it serves the last cached version instead of showing an error.
This way, someone checking their ticket board from a location with poor connectivity still
sees something useful. For write operations that happen while offline --- updating a ticket
status, for example --- the \textbf{Background Sync API} takes over: the pending request
is stored locally in IndexedDB, and as soon as the connection comes back, the Service
Worker picks it up and sends it to the server automatically.

\textbf{Push notifications.} The \textbf{Web Push API} lets the server send a notification
to the user's device even when the app is not open. In Canny TMS, this is wired to
assignment events: when a ticket is assigned to a team member, they get a notification
right away instead of having to refresh the board periodically.

Both features are built entirely on native browser APIs. No additional JavaScript library
or framework is needed, which keeps the zero-dependency policy intact.
...
On the technology side, Jakarta EE on WildFly was selected for its standards compliance, predictable CDI behaviour, and because Undertow removes the need for a separate reverse proxy. Vanilla JS with ES Modules keeps the frontend under full internal control with no supply chain exposure, and the PWA layer adds offline access and push notifications using nothing but standard browser APIs.
